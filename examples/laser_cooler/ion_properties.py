...
class Ion:
    def __init__(self, name, A, Z, q0, excited_lifetime, hw0,
                 ...
                 gamma_cooling,gamma_heating,theta_l,pulse_energy):
...
lead = Ion(name="Pb$^{79+}$", A=208, Z=82, q0=79, excited_lifetime=76.6e-12, hw0=230.823,
           ...
           theta_l=2.6*np.pi/180 , pulse_energy=5*1e-3,
...
calcium = Ion(name="Ca$^{17+}$", A=40, Z=20, q0=17, excited_lifetime=0.4279*1e-12, hw0=661.89,
              lambda_l=768*1e-9,laser_x=-1.577181208053691*1e-3, bunch_length=9.5*1e-2, bunch_intensity = 4*1e9,
               theta_l=1.3*np.pi/180 , pulse_energy=2*1e-3,
...
xenon2nd = Ion(name="Xe$^{51+}$ 2nd harmonic", A=129, Z=54, q0=51, excited_lifetime=3*1e-12, hw0=492.22,
               lambda_l=515*1e-9,laser_x=-1.3087248322147649*1e-3, bunch_length=0.082, bunch_intensity = 0.9*1e8,
               theta_l=1.3*np.pi/180 , pulse_energy=2*1e-3,
